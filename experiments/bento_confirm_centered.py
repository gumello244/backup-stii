...
# Established Remos colors matching the design system
RM_BG = "#FFFFFF"
RM_SURFACE = "#F9F9F9"
RM_BORDER = "#EEEEEE"
RM_TEXT = "#1A202C"
...
class BentoBox(QFrame):
    ...
    def update_content(self, title: str, value: str, subtitle: str) -> None:
        """Update content inside bento card components dynamically."""
        self._t_lbl.setText(title)
        self._v_lbl.setText(value)
        self._s_lbl.setText(subtitle)

    def set_default_style(self) -> None:
        """Reset styling to the default BentoCard visual mode."""
        self.setObjectName("BentoCard")
        self._v_lbl.setObjectName("BentoValue")
        self.style().unpolish(self)
        self.style().polish(self)
        self._v_lbl.style().unpolish(self._v_lbl)
        self._v_lbl.style().polish(self._v_lbl)

    def set_success_style(self) -> None:
        """Transition visual style to Success green hero card."""
        self.setObjectName("BentoCardHeroSuccess")
        self._v_lbl.setObjectName("BentoValueHeroSuccess")
        self.style().unpolish(self)
        self.style().polish(self)
        self._v_lbl.style().unpolish(self._v_lbl)
        self._v_lbl.style().polish(self._v_lbl)
    ...
    def set_danger_style(self) -> None:
        """Transition visual style to Danger red hero card."""
        self.setObjectName("BentoCardHeroDanger")
        self._v_lbl.setObjectName("BentoValueHeroDanger")
        self.style().unpolish(self)
        self.style().polish(self)
        self._v_lbl.style().unpolish(self._v_lbl)
        self._v_lbl.style().polish(self._v_lbl)
...
class BentoGrid(QWidget):
    ...
    def add_card(
        self, card: QWidget, row: int, col: int, rowspan: int = 1, colspan: int = 1
    ) -> None:
        """Add a bento widget block into the layout grid coordinates."""
        self._layout.addWidget(card, row, col, rowspan, colspan)


class BentoConfirmCenteredPrototype(QMainWindow):
    """BentoConfirmCenteredPrototype: Centered confirm view with PopoverCard wrapping."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Remos — Confirm Bento Centered")
    ...
    def _init_ui(self) -> None:
        ...
        self._setup_card_container(main_layout)
    ...
    def _setup_state_selector(self, parent_layout: QVBoxLayout) -> None:
        ...
        title = QLabel("Confirmar Centrado — Simular:")
    ...
    def _setup_card_container(self, parent_layout: QVBoxLayout) -> None:
        body_container = QWidget()
        body_layout = QHBoxLayout(body_container)
        body_layout.setContentsMargins(36, 12, 36, 16)
        ...
        self._setup_bento_grid(card_layout)
        self._setup_folder_list(card_layout)
        self._setup_bottom_buttons(card_layout)
    ...
    def _setup_card_header(self, layout: QVBoxLayout) -> None:
        ...
        subtitle = QLabel("Selecione as pastas e valide os requisitos para restaurar.")
    ...
    def _setup_bento_grid(self, layout: QVBoxLayout) -> None:
        ...
        self._hero_status = BentoBox("VERIFICAÇÃO DE ESPAÇO", "", "")
    ...
    def _setup_folder_list(self, layout: QVBoxLayout) -> None:
        links = QHBoxLayout()
        self._add_link_button(links, "Selecionar tudo", self._select_all)
        self._add_link_button(links, "Deselecionar tudo", self._deselect_all)
        links.addStretch()
        ...
        scroll_lay.setContentsMargins(12, 8, 12, 8)
        scroll_lay.setSpacing(6)

        self._populate_scroll_folders(scroll_lay)
        scroll.setWidget(scroll_content)
        layout.addWidget(scroll, stretch=1)

    def _add_link_button(self, layout: QHBoxLayout, text: str, slot: Callable[[], None]) -> None:
    ...
    def _populate_scroll_folders(self, layout: QVBoxLayout) -> None:
        folders = [
    ...
    def _on_state_changed(self, state_id: int) -> None:
        if state_id == 0:
            self._hero_status.update_content("VERIFICAÇÃO DE ESPAÇO", "Suficiente", "Disco rígido pronto")
            self._hero_status.set_success_style()
            self._restore_btn.setEnabled(True)
        else:
            self._hero_status.update_content("VERIFICAÇÃO DE ESPAÇO", "Insuficiente", "Liberar espaço em C:\\")
            self._hero_status.set_danger_style()
            self._restore_btn.setEnabled(False)

    def _select_all(self) -> None:
        """Select all folder checkboxes in the list."""
        for cb in self._checkboxes:
            cb.setChecked(True)

    def _deselect_all(self) -> None:
        """Deselect all folder checkboxes in the list."""
        for cb in self._checkboxes:
            cb.setChecked(False)
...
if __name__ == "__main__":
    ...
    window = BentoConfirmCenteredPrototype()
    window.show()
    sys.exit(app.exec_())
