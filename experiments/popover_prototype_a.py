...
class PopoverPrototypeA(QMainWindow):
    """Popover Prototype A: Centered modal card featuring Remos styling."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Remos — Popover A (Remos Palette)")
    ...
    def _setup_state_selector(self, parent_layout: QVBoxLayout) -> None:
        ...
        title = QLabel("Popover A — Estado do Teste:")
    ...
    def _setup_card_header(self, layout: QVBoxLayout) -> None:
        ...
        self._subtitle = QLabel("O Remos está analisando as fontes de backup disponíveis.")
    ...
    def _update_ui_state(self, state_id: int) -> None:
        ...
        if state_id == 0:
            self._show_loading("Localizando diretórios de backup...")
        elif state_id == 1:
            self._show_loading("Consolidando as versões dos arquivos...")
        elif state_id == 2:
            self._show_error()
        elif state_id == 3:
            self._show_resolved()
    ...
    def _show_error(self) -> None:
        ...
        err_title = QLabel("Nenhum Backup Localizado")
        err_title.setStyleSheet(f"color: {RM_RED}; font-size: 16px; font-weight: 700;")
        err_title.setAlignment(Qt.AlignCenter)

        err_desc = QLabel("Por favor, verifique a conexão com a rede local e tente novamente.")
    ...
    def _show_resolved(self) -> None:
        ...
        capsule = QLabel("Último Backup Encontrado: 24/06/2026")
        capsule.setObjectName("StatusCapsule")

        desc = QLabel("9 arquivos em 3 pastas  •  2,3 GB consolidados")
