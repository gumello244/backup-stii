...
class ExpressiveTypoB(QMainWindow):
    """ExpressiveTypoB: Unified popover card featuring a typographic narrative."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Remos — Expressive Typo B")
    ...
    def _setup_state_selector(self, parent_layout: QVBoxLayout) -> None:
        ...
        title = QLabel("Hero Typo B — Estado do Teste:")
    ...
    def _setup_card_header(self, layout: QVBoxLayout) -> None:
        self._title = QLabel("Consolidação Técnica")
        self._title.setObjectName("ViewTitle")
        self._title.setAlignment(Qt.AlignCenter)
        layout.addWidget(self._title)

        self._subtitle = QLabel("Resultados obtidos através dos repositórios.")
    ...
    def _update_ui_state(self, state_id: int) -> None:
        ...
        if state_id == 0:
            self._show_loading("Verificando estrutura de arquivos...")
        elif state_id == 1:
            self._show_loading("Mapeando últimas alterações técnicas...")
        elif state_id == 2:
            self._show_error()
        elif state_id == 3:
            self._show_resolved()
    ...
    def _show_error(self) -> None:
        ...
        err_title = QLabel("Backup não localizado")
        err_title.setStyleSheet(f"color: {RM_RED}; font-size: 17px; font-weight: 700;")
        err_title.setAlignment(Qt.AlignCenter)

        err_desc = QLabel("Verifique se seu computador está conectado à rede da empresa.")
    ...
    def _show_resolved(self) -> None:
        ...
        lbl = QLabel()
        lbl.setObjectName("ParagraphLabel")
        lbl.setAlignment(Qt.AlignCenter)
        lbl.setWordWrap(True)

        text = (
            "<html><body style='line-height: 1.6; color: #1A202C; text-align: center;'>"
            "Localizamos <span style='color: #3B6EA5; font-size: 17px; font-weight: 800;'>2.3 GB</span> "
            "de arquivos salvos no dia <span style='font-weight: 700;'>24/06/2026</span>.<br><br>"
            "Este backup contém <span style='font-weight: 700;'>9 arquivos</span> distribuídos "
            "em <span style='font-weight: 700;'>3 pastas</span> estruturadas para o seu usuário."
            "</body></html>"
        )
        lbl.setText(text)
