...
class BentoPrototypeB(QMainWindow):
    """BentoPrototypeB: Symmetric bento column grid inside a unified popover card."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Remos — Bento B (Symmetric Bento Grid)")
    ...
    def _setup_state_selector(self, parent_layout: QVBoxLayout) -> None:
        ...
        title = QLabel("Bento B — Estado do Teste:")
    ...
    def _setup_card_header(self, layout: QVBoxLayout) -> None:
        self._title = QLabel("Análise Técnica Concluída")
        self._title.setObjectName("ViewTitle")
        self._title.setAlignment(Qt.AlignCenter)
        layout.addWidget(self._title)

        self._subtitle = QLabel("Dados de backup localizados e estruturados.")
    ...
    def _update_ui_state(self, state_id: int) -> None:
        ...
        if state_id == 0:
            self._show_loading("Localizando backups do usuário...")
        elif state_id == 1:
            self._show_loading("Analisando as versões dos arquivos...")
        elif state_id == 2:
            self._show_error()
        elif state_id == 3:
            self._show_resolved()
    ...
    def _show_error(self) -> None:
        ...
        err_desc = QLabel("Não detectamos fontes. Verifique os cabos de rede e tente novamente.")
    ...
    def _show_resolved(self) -> None:
        ...
        # Bento 1: Date
        box_left = self._create_bento_box("ÚLTIMO BACKUP", "24/06", "Gerado em 2026", False)
        # Bento 2: Hero Size (Symmetric Highlight)
        box_center = self._create_bento_box("TAMANHO TOTAL", "2.3 GB", "Volume de dados", True)
        # Bento 3: Files
        box_right = self._create_bento_box("ESTRUTURA", "9 itens", "Em 3 pastas principais", False)

        lay.addWidget(box_left)
        lay.addWidget(box_center)
        lay.addWidget(box_right)

        self._state_layout.addWidget(row)
