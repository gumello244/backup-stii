...
class BentoRedesignB(QMainWindow):
    """BentoRedesignB: Asymmetrical grid with high-contrast minimalist details."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Remos — Bento Redesign B")
    ...
    def _setup_state_selector(self, parent_layout: QVBoxLayout) -> None:
        ...
        title = QLabel("Redesign B — Estado:")
    ...
    def _setup_card_header(self, layout: QVBoxLayout) -> None:
        self._title = QLabel("Restauração de Arquivos")
        self._title.setObjectName("ViewTitle")
        layout.addWidget(self._title)

        self._subtitle = QLabel("Processo de identificação e integridade do backup.")
    ...
    def _show_loading_discovering(self) -> None:
        """Show discovering state with flat dotted outline placeholders."""
        self._next_btn.setEnabled(False)
        self._subtitle.setText("Pesquisando origens de dados de backup...")
        ...
        spinner = Spinner()
        lbl = QLabel("Procurando fontes no servidor e disco local...")
    ...
    def _show_loading_merging(self) -> None:
        ...
        self._next_btn.setEnabled(False)
        self._subtitle.setText("Comparando as versões disponíveis dos arquivos...")
        ...
        # Pulses
        box_top = self._create_skeleton_box("ESTRUTURA DE BACKUP", "Aguardando consolidação técnica...")
        lay.addWidget(box_top, 0, 0, 1, 2)
        ...
        l_lay.addWidget(QLabel("Consolidando..."))
        lay.addWidget(box_left, 1, 0, 1, 1)
    ...
    def _show_error(self) -> None:
        ...
        t_lbl = QLabel("ÚLTIMO BACKUP  •  🔴 Desconectado")
        t_lbl.setObjectName("BentoTitle")
        t_lbl.setStyleSheet(f"color: {RM_RED};")
        v_lbl = QLabel("Backup inacessível")
        v_lbl.setObjectName("BentoValueHero")
        v_lbl.setStyleSheet(f"color: {RM_TEXT};")
        s_lbl = QLabel("Verifique os cabos de rede e o sinal de rede local.")
        s_lbl.setObjectName("BentoSub")

        h_lay.addWidget(t_lbl)
        h_lay.addWidget(v_lbl)
        ...
        retry_btn = QPushButton("Reconectar")
    ...
    def _show_resolved(self) -> None:
        ...
        self._subtitle.setText("Estrutura de dados pronta para restauração.")
        ...
        # Bento 1: Top Hero Date (Spans 2 columns) with status dot in title
        hero = self._create_bento_box("ÚLTIMO BACKUP  •  🟢 Saudável", "24 de Junho de 2026", "Consolidado com integridade", True)
        lay.addWidget(hero, 0, 0, 1, 2)

        # Bento 2: Size
        size = self._create_bento_box("TAMANHO TOTAL", "2.3 GB", "Espaço necessário em disco", False)
        lay.addWidget(size, 1, 0, 1, 1)

        # Bento 3: Files
        files = self._create_bento_box("ESTRUTURA", "9 arquivos", "Separados em 3 pastas", False)
